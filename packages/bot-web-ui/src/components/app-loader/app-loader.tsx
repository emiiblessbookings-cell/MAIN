...
const GlobalLoading: React.FC<GlobalLoadingProps> = ({ 
    ...
    useEffect(() => {
        ...
        // Progress timer - FIXED: This now properly completes and triggers disappearance
        const progressInterval = setInterval(() => {
            setProgress(prev => {
                const newProgress = prev + (100 / (duration / 100)); // Calculate increment based on duration
                if (newProgress >= 100) {
                    clearInterval(progressInterval);
                    clearInterval(marketInterval);
                    
                    // FIXED: Add the completion logic here
                    setTimeout(() => {
                        setIsVisible(false);
    ...
    }, [isMobile, duration, onLoadingComplete, controls]);
    ...
    // FIXED: Return null when not visible, just like the first example
